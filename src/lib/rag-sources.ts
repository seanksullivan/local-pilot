...
export interface RagSourceSystemPreset {
  label: string;
  text: string;
}
...
export interface RagSourceConfig {
  ...
  systemPrompt: string;
  /** Sample system-prompt buttons shown in the composer. */
  systemPresets?: RagSourceSystemPreset[];
}
...
export const RCW_TITLE_11_TABLE_PRESET = `You are a professional specializing in Washington State probate and trust law (RCW Title 11). Cite statutes when relevant. This is educational information, not formal legal advice.
- Put information in a table format if possible`;
...
export const RAG_SOURCES: RagSourceConfig[] = [
  {
    ...
    systemPresets: [
      { label: 'Probate', text: RCW_TITLE_11_PRESET },
      { label: 'Probate table', text: RCW_TITLE_11_TABLE_PRESET },
    ],
  },
];
